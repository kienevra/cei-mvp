...
class PartnerInvite(Base):
    """
    Invite token generated by a commercialista (managing org) to onboard
    a factory client. When the factory follows the link and signs up,
    their org is automatically linked with managed_by_org_id set to the
    commercialista org.

    Token lifecycle:
        created  -> expires_at not reached, used_at is None, revoked_at is None
        used     -> used_at is set, used_by_org_id is set
        expired  -> expires_at < now(), used_at is None
        revoked  -> revoked_at is set

    Only one active use per token (unique token_hash index).
    """
    __tablename__ = "partner_invite"

    id               = Column(Integer,              primary_key=True)
    managing_org_id  = Column(Integer,              ForeignKey("organization.id", ondelete="CASCADE"), nullable=False)
    token            = Column(String(64),           nullable=False)
    token_hash       = Column(String(64),           nullable=False, unique=True, index=True)
    factory_name     = Column(String(255),          nullable=True)
    factory_email    = Column(String(255),          nullable=True)
    note             = Column(Text,                 nullable=True)
    created_at       = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    expires_at       = Column(DateTime(timezone=True), nullable=False)
    used_at          = Column(DateTime(timezone=True), nullable=True)
    used_by_org_id   = Column(Integer,              ForeignKey("organization.id", ondelete="SET NULL"), nullable=True)
    revoked_at       = Column(DateTime(timezone=True), nullable=True)

    managing_org = relationship("Organization", foreign_keys=[managing_org_id],
                                backref="partner_invites")
    used_by_org  = relationship("Organization", foreign_keys=[used_by_org_id])
