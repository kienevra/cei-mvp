...
def signup(user: UserCreate, db: Session = Depends(get_db)):
  ...
  if user.org_id is not None and user.organization_id is None:
    ...
    logger.warning(
      "Received deprecated payload field `org_id`; prefer `organization_id` (will be removed in future)."
    )
...
def delete_me(
  ...
  In this MVP we rely on SQLAlchemy relationships (subscriptions, etc.)
  to cascade. After deletion, any existing JWT will immediately start
  failing because `get_current_user` will no longer find the user.
  """
