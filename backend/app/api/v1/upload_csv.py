...
# Core semantic columns CEI expects
REQUIRED_COLUMNS = {"timestamp", "value", "unit", "site_id", "meter_id"}
...
def _ensure_required_columns(canonical_map: dict):
    """
    Validate that the canonical header set contains all REQUIRED_COLUMNS.
    Raises HTTPException with structured detail if any are missing.
    """
    present = set(canonical_map.values())
    missing = sorted(list(REQUIRED_COLUMNS - present))
    if missing:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    ...
                    "Expected at least: timestamp, value, unit, site_id, meter_id."
                ),
                "missing": missing,
            },
        )
...
async def upload_csv(
    ...
    Expected semantic columns (header row):
    ...
    _ensure_required_columns(canonical_map)
